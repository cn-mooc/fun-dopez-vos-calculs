...
void internalComput(int nll2, int nlc2, double *x, double *oldx, double h2, double *f)
{
   for (int i = 2; i < nll2 - 2; i++)
   {
      for (int j = 2; j < nlc2 - 2; j++)
      {
         int p = i * nlc2 + j;
         x[p] = 0.25 * (h2 * f[p] + (oldx[p + 1] + oldx[p - 1] + oldx[p + nlc2] + oldx[p - nlc2]));
      }
...
void write_vti(int rank, int nc, int n, double h, const vector<double> &values, const string &file_name = "toto")
{
   int bx = rank % nc * n;
   int by = rank / nc * n;
   ...
   os << "<ImageData WholeExtent=\"0 " << nc * n - 1 << " 0 " << nc * n - 1 << " 0 0\" Origin=\"0.0 0.0 0.0\" Spacing=\"" << h
      << " " << h << " " << h << "\">" << endl;
   os << "<Piece Extent=\"" << bx << " " << bx + n - 1 << " " << by << " " << by + n - 1 << " 0 0\">" << endl;
...
int main(int argc, char **argv)
{
   ...
   */
   ...
   while (error > epsilon && iter < max_iter)
   {
      ...
      */
      bndComput(nlc, nlc2, nll, nll2, h2, f.data(), &(x[0]), oldx.data());
      ...
      */
      internalComput(nll2, nlc2, &(x[0]), &(oldx[0]), h2, &(f[0]));
   ...
   //  write_vti(rank,nc,nl2,h,x,"toto_mpi");
